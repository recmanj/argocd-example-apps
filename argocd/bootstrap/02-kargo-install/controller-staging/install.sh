...
echo "=== Kargo Distributed Controller Installation (Staging Cluster) ==="
echo ""
...
read -p "Is this the STAGING cluster? (y/N) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "Please switch to the staging cluster context first:"
    echo "  kubectl config use-context <staging-cluster-context>"
    exit 1
fi
...
if ! kubectl -n kargo get secret kargo-controlplane-kubeconfig &> /dev/null; then
    ...
    echo "  ./deploy-secret.sh staging    # Run this in STAGING cluster"
    exit 1
fi

echo "Installing Kargo controller (staging-shard) via Helm..."
...
echo ""
echo "========================================="
echo "Kargo controller (staging-shard) installation complete!"
echo "========================================="
echo ""
echo "Controller configuration:"
echo "  Shard name: staging-shard"
...
echo "     kubectl -n argocd-example-apps get stages -o yaml | grep 'shard: staging-shard'"
echo ""
